...
import { droppedFiles, importFiles, pickAndImport, insertAssetAtPlayhead } from "@/engine/asset-actions";
import { isInputTarget } from "@/utils";
import { AlertDialog, AlertDialogPortal, AlertDialogContent, AlertDialogTitle, AlertDialogDescription, AlertDialogFooter } from "../ui/alert-dialog";

import type { AssetEntry, AssetLibrary } from "@diffusionstudio/assets";
...
export function Assets() {
  ...
  const [pendingDelete, setPendingDelete] = createSignal<{ entry: AssetEntry; library: AssetLibrary } | null>(null);
  const [deleting, setDeleting] = createSignal(false);
  let cancelDelete: HTMLButtonElement | undefined;
  const requestDelete = (entry: AssetEntry) => {
    const owner = library();
    if (owner && !pendingDelete()) setPendingDelete({ entry, library: owner });
  };
  const confirmDelete = async () => {
    ...
    setDeleting(true);
    try {
      await pending.library.remove([pending.entry]);
      if (selectedAssetId() === pending.entry.id) setSelectedAssetId(null);
      setPendingDelete(null);
    } catch (error) {
      toast.error('Failed to delete media', { description: (error as Error).message });
  ...
  const selectedAsset = createMemo(() => filteredAssets().find(asset => asset.id === selectedAssetId()));
  ...
  const handleImportAssets = () => withLibrary(async lib => {
    const assets = await pickAndImport(lib, currentFolder());
    if (assets.length) {
      setQuery("");
      setAssetFilter("ALL");
      handleSelectAsset(assets[0].id);
    }
  });
  ...
  const handleDrop = async (event: DragEvent) => {
    ...
    const assets = await importFiles(lib, droppedFiles(event), currentFolder());
    if (assets.length) {
      setQuery("");
      setAssetFilter("ALL");
      handleSelectAsset(assets[0].id);
    ...
    >
      <div class="h-12 shrink-0 flex items-center gap-2 px-4 border-y border-border">
        <div class="flex-1 min-w-0 flex items-center gap-0.5 text-[12px] leading-5 font-strong text-foreground">
          ...
          <span class="truncate" data-panel-title>
            {panelTitle()}
            ...
            <div class="h-full flex flex-col items-center justify-center gap-4 px-6 pb-[88px] pt-4">
              <div class="flex flex-col items-center gap-1">
                ...
                <p class="text-xxs text-muted-foreground text-center">
                  Drag here or import media from your computer.
                </p>
              </div>
              <div class="flex flex-col gap-2 w-full">
                <Show when={!import.meta.env.VITE_PURECUT}><Button variant="secondary" class="w-full" onClick={() => openPromptInput(createDefaultConfig("IMAGE"))}>
                  Generate with AI
                </Button></Show>
      ...
      <div class="shrink-0 border-t border-border p-3 flex flex-col gap-2">
        <span class="truncate text-xs text-muted-foreground" title={selectedAsset()?.path}>
          {selectedAsset() ? basename(selectedAsset()!.path) : 'No media selected'}
        </span>
        <Button
          variant="secondary"
          class="w-full gap-2"
          disabled={!selectedAsset() || !!selectedAsset()?.sourceError}
          onClick={() => {
            const asset = selectedAsset();
            if (asset && !asset.sourceError) insertAssetAtPlayhead(world, asset);
          }}
        >
          <Icon name="plus-add" class="size-4 shrink-0" />
          Insert at playhead
        </Button>
      </div>
      <AlertDialog open={pendingDelete() !== null} onOpenChange={open => {
        ...
        <AlertDialogContent onOpenAutoFocus={event => {
          ...
          <AlertDialogDescription>
            {`Delete "${pendingDelete()?.entry.path ?? ''}" from the library? Its project-owned file will be permanently removed. Clips using it may no longer play or export. This cannot be undone.`}
          </AlertDialogDescription>
          <AlertDialogFooter>
            <Button ref={cancelDelete} variant="secondary" disabled={deleting()} onClick={() => setPendingDelete(null)}>Cancel</Button>
            <Button variant="destructive" disabled={deleting()} onClick={confirmDelete}>{deleting() ? 'Deleting...' : 'Delete media'}</Button>
          </AlertDialogFooter>
        </AlertDialogContent>
        </AlertDialogPortal>
      </AlertDialog>
