...
export function SceneInitOverlay() {
  ...
  /**
   * The placeholder used to be a grey frame you had to guess was a button.
   * These are the two ways a video actually starts, said out loud.
   */
  const handleImportIntoNewScene = async () => {
    const library = world.get(Library);
    if (!library) return;
    const assets = await pickAndImport(library, '');
    if (assets?.length) insertAssetsInNewScene(world, assets, sceneOptions());
    else handleInitializeScene();
  };
        ...
        >
          ...
          >
            <div class="flex flex-col items-center gap-3 px-6 text-center">
              <span class="text-sm font-450 text-foreground">Start this video</span>
              <span class="text-xs text-muted-foreground max-w-[340px]">
                Bring in clips, or open an empty {selectedPreset().label.toLowerCase()} scene and
                build on it. You can also drop files here.
              </span>
            </div>
          </button>
          <div class="absolute left-0 right-0 top-full mt-3 flex items-center justify-center gap-2">
            <Button variant="default" onClick={() => void handleImportIntoNewScene()}>
              <Icon name="download" class="size-6" />
              Import media
            </Button>
            <Button variant="ghost" onClick={handleInitializeScene}>
