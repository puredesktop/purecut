...
import { SubtitleExport } from './subtitle-export';
import {
...
/**
 * How one scene is exported, and the edits the UI makes to it. Project
 * configuration rather than part of the composition: read from and written
 * through the project config (package.json `diffusion.export.<scene id>`,
 * see `@/engine/project-config`). The panel, the row, the floating inspector
 * and the standalone button all read the same accessors, so they never
 * disagree.
 */
function createSceneExport(entity: Accessor<Entity | undefined>) {
  ...
  const settings = createMemo(() => {
    ...
    return config()?.exportOf(current) ?? undefined;
  });
  ...
  const write = (value: ProjectExportConfig | null) => {
    const current = entity();
    if (!current) return;
    void config()?.setExport(current, value);
  };
  ...
  // The settings a scene without any starts from: the same default the file
  // menu exports with.
  const ensureSettings = () => {
    if (settings()) return;
    applyTemplate(getDefaultExportTemplate().id);
  };
...
/** How the selected scene is exported, and the way to change it. */
export function ExportPanel(props: ExportPanelProps) {
  const {
    ...
    removeSettings,
    runExport,
  } = createSceneExport(() => props.selection[0]);
  ...
  return (
    <PanelSection title="Export" ref={inspectorAnchorRef}>
      <Show when={props.selection[0]}>{scene => <SubtitleExport scene={scene()} />}</Show>
      {/* Nothing set up yet: one button that makes the default settings and
          opens them, rather than an icon that has to be found first. */}
      <Show when={!settings()}>
        <Button class="w-full" onClick={addSettings}>
          Export video
        </Button>
      </Show>
      <Show when={settings()}>
...
export type ExportVideoButtonProps = {
  /** The scene to export. The active one when not given. */
  scene?: Entity;
  /** The button's text. "Export video" when not given. */
  label?: string;
  class?: string;
...
/**
 * One button that gets to an export, wherever it is put: it makes the default
 * settings for a scene that has none and opens the same settings dialog the
 * export panel opens, so a first export is one click. Everything it needs it
 * reads off the world, so any caller inside the editor can render it.
 */
export function ExportVideoButton(props: ExportVideoButtonProps) {
  const activeScene = useActiveScene();
  const scene = () => props.scene ?? activeScene() ?? undefined;
  ...
  return (
    <div class={cx("w-full", props.class)} ref={anchorRef}>
      <Button
        class="w-full"
...
type ExportInspectorProps = {
  ...
  /** Rendered under the settings — for callers with no export row of their own. */
  footer?: JSX.Element;
};
    ...
    <FloatingInspector open anchorRef={props.anchorRef} width={272}>
      ...
      <FloatingInspectorContent class="p-4">
        ...
        <Show when={props.footer}>
          <Separator class="my-3" />
          <div class="flex flex-col gap-2">{props.footer}</div>
        </Show>
      </FloatingInspectorContent>
    </FloatingInspector>
  );
