...
async function ensureModuleLoaded(ctx: BaseAudioContext) {
  ...
  if (!pending) {
    pending = ctx.audioWorklet.addModule(getVolumeMeterWorkletUrl()).catch(error => {
...
function detach(gainNode: GainNode, listener: Listener) {
  ...
  try { gainNode.disconnect(shared.workletNode); } catch { /* already disconnected */ }
...
export function useVolumeMeter() {
  ...
  const connect = async (gainNode: GainNode) => {
    if (currentGainNode === gainNode) return;
    ...
    try {
      const release = await subscribe(gainNode, next => {
        if (token === subscriptionToken) {
          setLevels(next);
          if (next.some(channel => channel.peak >= 1)) setClipped(true);
        }
      });
      // Each request has its own listener: releasing an old subscription
      // must not detach the replacement that shares the same gain node.
      if (token !== subscriptionToken) {
        release();
        return;
      }
  ...
  return { levels, error, clipped, resetClipping: () => setClipped(false), connect, disconnect };
}
    ...
    process(inputs: Float32Array[][], outputs: Float32Array[][]) {
      ...
      if (this.framesSinceLastPost >= this.updateInterval) {
        ...
        for (let ch = 0; ch < channels; ch++) {
          ...
          data[ch * 2 + 1] = Math.max(this.smoothedPeak[ch], this.peakHoldValues[ch]);
        }
