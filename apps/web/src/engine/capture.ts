 ...
 */
async function bundleFor(source: World, dir?: string): Promise<{ code: string; sourceHash?: string }> {
	if (dir && isDesktop()) {
...
export interface Capture {
	/** Fingerprint of the exact source compiled for this capture, when available. */
	sourceHash?: string;
...
export async function createCapture(source: World, node: Entity, options: CaptureOptions = {}): Promise<Capture> {
	const { code, sourceHash } = await bundleFor(source, options.dir);
	...
	return { world, node: rendered, sourceHash, dispose: dispose(mounted) };
}
