...
import { rippleDeleteSelection } from '../ripple-delete';
...
/** Seek through visible clip boundaries within the current work area. */
export function seekToCut(world: World, direction: -1 | 1): void {
	const scene = getActiveEntity(world);
	if (!scene) return;
	const computed = store(world, Computed);
	const area = scene.get(Workarea);
	const start = area?.start ?? 0;
	const end = area?.end ?? computed.duration[scene.id()] ?? 0;
	...
	const visit = (parent: Entity) => {
		for (const child of getEntityChildren(world, parent)) {
			...
			for (const edge of [computed.start[child.id()], computed.end[child.id()]])
				if (edge !== undefined && Number.isFinite(edge) && edge >= start && edge <= end) edges.add(edge);
			if (isGroupLike(child)) visit(child);
		}
	};
	visit(scene);
	const current = computed.localTime[scene.id()] ?? 0;
	const candidates = [...edges].filter(frame => direction > 0 ? frame > current : frame < current);
	if (!candidates.length) return;
	stopPlayback(world, scene);
	setPlayhead(world, scene, direction > 0 ? Math.min(...candidates) : Math.max(...candidates));
}
