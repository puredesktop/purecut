...
import { containsLocked, isEditLocked } from './locking';
...
export class DocumentEditor {
	...
	public replayEdits(action: () => void): void {
		this.replayDepth++;
		try { action(); } finally { this.replayDepth--; }
	}

	private protected(entity: Entity): boolean {
		return this.replayDepth === 0 && containsLocked(this.world, entity);
	}

	public editProperty(entity: Entity, name: string, value: PropValue | AssetRef): void {
		if (!['locked', 'expanded', 'clipHeight', 'selected', 'playhead'].includes(name) && this.protected(entity)) return;
		...
		if (name === 'locked' && value === true) {
			for (const selected of [...this.world.query(Selected)]) {
				if (containsLocked(this.world, selected)) this.setSelected(selected, false);
			}
		}
	}
	...
	public select(entities: Entity | Entity[], options: { extend?: boolean } = {}): void {
		const next = new Set((Array.isArray(entities) ? entities : [entities]).filter(entity => !this.protected(entity)));
	...
	public insertElement(parent: Entity, element: () => unknown, anchor?: Entity): Entity[] {
		if (!this.replayDepth && isEditLocked(parent)) return [];
	...
	public wrap(entities: Entity | Entity[], element: () => unknown): Entity | null {
		const roots = new Set(this.subtreeRoots(entities).filter(entity => !this.protected(entity)));
		const first = [...roots][0];
	...
	public reparent(entity: Entity, parent: Entity, anchor?: Entity): boolean {
		if (this.protected(entity) || (!this.replayDepth && isEditLocked(parent))) return false;
