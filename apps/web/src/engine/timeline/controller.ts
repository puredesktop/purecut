...
export function createTimelineController(world: World) {
	...
	const fit = (): void => {
		withScene(scene => {
			const width = surface.canvas?.clientWidth ?? 0;
			if (width <= 2 * TIMELINE_PADDING_LEFT) return;
			const duration = scene.get(Computed)?.duration ?? 0;
			const next = clamp(
				(width - 2 * TIMELINE_PADDING_LEFT) / Math.max(1, duration),
				1 / TIMELINE_RESOLUTION_RANGE[1],
				1 / TIMELINE_RESOLUTION_RANGE[0],
			);
			setResolution(world, scene, next);
			setScrollX(world, scene, -TIMELINE_PADDING_LEFT / next);
			updateTimelineTransform(world, scene);
			reportView(scene);
		});
	};

	const zoomAtPlayhead = (factor: number): void => {
		if (!Number.isFinite(factor) || factor <= 0) return;
		withScene(scene => {
			const resolution = getResolution(world, scene);
			const frame = scene.get(Computed)?.localTime ?? 0;
			const width = surface.canvas?.clientWidth ?? 0;
			const position = (frame - getScrollX(world, scene)) * resolution;
			const visible = position >= 0 && position <= width;
			const anchor = visible ? position : width / 2;
			const anchorFrame = visible ? frame : getScrollX(world, scene) + anchor / resolution;
			const next = clamp(resolution * factor, 1 / TIMELINE_RESOLUTION_RANGE[1], 1 / TIMELINE_RESOLUTION_RANGE[0]);
			setResolution(world, scene, next);
			setScrollX(world, scene, anchorFrame - anchor / next);
			updateTimelineTransform(world, scene);
			reportView(scene);
		});
	};
