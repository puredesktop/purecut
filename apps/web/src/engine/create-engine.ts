...
import { assetSystem, renderSystem, transformSystem, playbackSystem, motionSystem, syncAudioBuses, AudioEngine, createRuntimeWorld, Geometry, Mode, RenderSurface, Time, ChildOf, syncInteractiveState } from '@diffusionstudio/runtime';
...
class Engine {
	...
	public failure: Accessor<string | null>;
	private setFailure: Setter<string | null>;
	...
	public constructor(projectId: string, options: EngineOptions = {}) {
		...
		[this.failure, this.setFailure] = createSignal<string | null>(null);
	...
	private readonly onKeyUp = (event: KeyboardEvent) => {
		...
		const wasHeld = keys.held.delete(key);
		if (wasHeld && !ownsKeyboardEvent(event)) keys.lifted.add(key);
	...
	private resize(): void {
		...
		if (!this.failure()) this.tryRunSystems();
	}

	private tryRunSystems(): boolean {
		try {
			this.runSystems();
			return true;
		} catch (error) {
			console.error('[PureCut] Preview stopped', error);
			this.stop();
			for (const scene of this.world.query(Playback)) stopPlayback(this.world, scene);
			this.world.get(Keys)?.held.clear();
			this.world.get(Keys)?.pressed.clear();
			this.world.get(Keys)?.lifted.clear();
			this.setFailure(error instanceof Error ? error.message : String(error));
			return false;
		}
	}
