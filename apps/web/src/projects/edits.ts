...
const activeWriters = new Set<EditWriter>();
export async function flushPendingProjectEdits(): Promise<void> {
 await Promise.all([...activeWriters].map(writer => writer.drain()));
}
...
class EditWriter {
 ...
 public async drain(): Promise<void> {
  for (let pass=0; pass<100; pass++) {
   clearTimeout(this.timer);this.flush();
   if (!this.writes.size) {if(this.writeError)throw this.writeError;return;}
   await Promise.all([...this.writes]);
  }
  throw Error("Project edits have not finished saving");
 }
 ...
 activeWriters.add(this);
	...
	public dispose(): void {
		...
		this.flush();
		this.disposed = true;
		// Remapping pending insert IDs can produce another write after unmount.
		// Keep global save barriers waiting, but never touch the retired world.
		void this.drain().catch(() => {}).finally(() => activeWriters.delete(this));
	}
	...
	private report(result: WriteResult): void {
		if (result.error) {
            this.writeError = new Error(result.error);
			toast.error('Could not write to the project', { description: result.error });
		}

		const ids = result.ids ?? {};
		const editor = this.disposed ? undefined : getDocumentEditor(this.world);
		editor?.restamp(ids);
