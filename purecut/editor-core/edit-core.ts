...
import type {
  EditValue,
  SourceContext as BaseSourceContext,
  SourceEdit,
  SourceInsert,
  SourceIteration,
  SourceMove,
...
export interface SourceContext extends BaseSourceContext {
  io: { read(file: string): Promise<string>; write(file: string, text: string): Promise<void>; files(): Promise<string[]> };
}
const randomInt = (max: number) => Math.floor(crypto.getRandomValues(new Uint32Array(1))[0] / 4294967296 * max);
...
class SourceWriter {
  ...
  private async load(path: string): Promise<void> {
    ...
    let text: string;
    try {
      text = await this.context.io.read(path);
    } catch {
      return;
    }
  ...
  private async save(): Promise<void> {
    for (const [path, open] of this.files) {
      ...
      await this.context.io.write(path, text);
    }
  }
...
export async function stampProject(context: SourceContext): Promise<void> {
  await new SourceWriter(context).stampProject(await context.io.files());
}
