...
function forwardCaptionDecoder(world: World, _scene: Entity, entity: Entity): void {
	...
	if (decoder.ready) {
		...
		// Offline frames must include captions even when the first cue starts at zero.
		framePromises(world)?.push(decoder.initPromise.then(() => {
			if (entity.isAlive() && entity.get(CaptionDecoderHandle) === decoder)
				decoder.seekTo(world, entity, localFrame / fps);
		}));
	}
}
...
/** Offline gain changes are scheduled after motion at their encoded timestamp. */
export function syncAudioBuses(world: World, atTime?: number): void {
	let soloed: Set<Entity> | null = null;
	for (const entity of world.query(AudioBusHandle)) {
		entity.get(AudioBusHandle)?.sync(atTime);
	...
	if (soloed) {
		for (const entity of world.query(AudioBusHandle)) {
			if (!soloed.has(entity) && !collectAncestors(entity).some(ancestor => ancestor.has(Soloed))) {
				entity.get(AudioBusHandle)?.mute(atTime);
			}
		}
	}
}
