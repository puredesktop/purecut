...
import { playbackAsset } from './playback-proxy';
...
export class VideoBuffer {
	...
	private async initialize() {
		try {
			...
			// dispose() may have run while the decoder was initializing.
			if ((this.mode as VideoBufferMode) === 'discarded') { this.queue.dispose(); return; }
...
export function getVideoTrack(source: VideoAsset) {
	...
	let promise = videoTrackCache.get(handle);
	if (promise) {
		return promise;
	}
	...
	promise = (async () => {
		try {
			...
			if (videoTrackCache.get(handle) === promise) videoTrackCache.delete(handle);
			return null;
		}
	})();

	videoTrackCache.set(handle, promise);
	return promise;
}
...
export function resolveVideoDecoder(world: World, entity: Entity): VideoDecoderInstance | null {
	...
	const original = getAsset(world, assetId);
	const asset = original?.type === 'VIDEO' && hasCache ? playbackAsset(original) : original;

	// A proxy changes the handle without changing the asset id. Mode changes
	// must also rebuild the decoder so an export never reuses a preview buffer.
	const existing = entity.get(VideoDecoderHandle);
	if (existing && existing.asset.id === assetId && existing.asset.handle === asset?.handle &&
		(!(existing instanceof VideoBuffer) || hasCache) && (!(existing instanceof VideoExporter) || !hasCache)) {
