...
import { AssetId, Caption, ChildOf, Paint, Shadow, Source, SourceError, TextRange, CaptionDecoderHandle } from '../../traits';
...
export function resolveCaptionDecoder(world: World, entity: Entity): CaptionDecoder | null {
	...
	void decoder.initPromise.catch(error => {
		if (!entity.isAlive() || entity.get(CaptionDecoderHandle) !== decoder) return;
		entity.add(SourceError);
		entity.set(SourceError, { value: error instanceof Error ? error.message : String(error), generated: false });
	});
