...
import {
	...
	assetSystem, playbackSystem, motionSystem, syncAudioBuses, transformSystem, renderSystem,
...
export async function createEncoder(world: World, config: EncoderConfig) {
	...
	// Embedded desktop iframes may not be cross-origin isolated. In that
	// case, advance OfflineAudioContext with suspend/resume checkpoints and
	// encode its final buffer, rather than requiring shared memory.
	const streamAudio = audioEnabled && typeof SharedArrayBuffer !== 'undefined';
	const sharedUint32Array = new Uint32Array(streamAudio ? new SharedArrayBuffer(4) : new ArrayBuffer(4));
	const mixNode = offlineAudioCtx.createGain();
	let audioWorkletUrl: string | undefined;
	let sinkNode: AudioWorkletNode | undefined;
	if (streamAudio) {
		audioWorkletUrl = createAudioWorkletUrl();
		await offlineAudioCtx.audioWorklet.addModule(audioWorkletUrl);
		sinkNode = new AudioWorkletNode(offlineAudioCtx, 'sink', {
			channelCount: numberOfChannels, channelCountMode: 'explicit',
			numberOfInputs: 1, numberOfOutputs: 1, outputChannelCount: [numberOfChannels],
			processorOptions: { buffer: sharedUint32Array },
		});
		mixNode.connect(sinkNode);
		sinkNode.connect(offlineAudioCtx.destination);
	} else {
		mixNode.connect(offlineAudioCtx.destination);
	}
	...
	if (sinkNode) sinkNode.port.onmessage = event => {
		...
		try {
			...
			for (let frame = 0; frame < totalFrames; frame++) {
...

				{
					...
					motionSystem(world);
					syncAudioBuses(world, timestamp);
				...
				if (audioEnabled && !streamAudio) {
					const nextTime = (frame + 1) * frameDuration;
					const checkpoint = frame + 1 < totalFrames && nextTime < duration
						? offlineAudioCtx.suspend(nextTime) : null;
					if (!audioRenderingCompleted) audioRenderingCompleted = offlineAudioCtx.startRendering();
					else await offlineAudioCtx.resume();
			...
			if (audioEnabled) {
				...
				if (streamAudio) {
					...
					const rendered = await audioRenderingCompleted;
					for (let offset = 0; offset < rendered.length; offset += 16384) {
						if (canceled) { await output.cancel(); return { type: 'canceled' }; }
						const count = Math.min(16384, rendered.length - offset);
						const data = new Float32Array(count * numberOfChannels);
						for (let channel = 0; channel < numberOfChannels; channel++)
							data.set(rendered.getChannelData(channel).subarray(offset, offset + count), channel * count);
						const sample = new AudioSample({ data, format: 'f32-planar', numberOfChannels, sampleRate, timestamp: offset / sampleRate });
			...
			if (audioWorkletUrl) URL.revokeObjectURL(audioWorkletUrl);
			// Silent exports never start this context. Resume only an actual
			// suspended render, and do not mask the export result with cleanup.
			if (audioRenderingCompleted && offlineAudioCtx.state === "suspended")
				await offlineAudioCtx.resume().catch(() => {});
		}
	};
